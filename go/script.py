...
def generate_article(topic: str, programming_language: str, article_language: str, max_retries: int = 5) -> tuple:
    """
    OpenAI API를 호출하여, 잘못된 정보 없이 정확한 내용을 담은 Go 명령어 문서(예: set command 수준의 상세 문서)를 생성합니다.
...
def main():
    ...
    # 위키로 만들 프로그래밍 언어 이름 (예: Go, Python, Java 등)
    programming_language = "Go"  # 원하는 프로그래밍 언어로 수정 가능
